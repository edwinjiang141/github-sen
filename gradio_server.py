...
from core.github_client import GitHubClient  # 导入用于GitHub API操作的客户端
# from hacker_news_client import HackerNewsClient
from reports.report_generator import ReportGenerator  # 导入报告生成器模块
from core.llm import LLM # 导入可能用于处理语言模型的LLM类
from models.subscription import load_subscriptions # 导入订阅管理器
from core.logger import LOG  # 导入日志记录器
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("GitHub 项目进展"):
        ...
        # 创建订阅列表的 Dropdown 组件
        subscription_list = gr.Dropdown(subscriptions, label="订阅列表", info="已订阅GitHub项目")
...
if __name__ == "__main__":
    demo.launch(share=True, server_name="0.0.0.0",server_port=6006)  # 启动界面并设置为公共可访问
    # 可选带有用户认证的启动方式
